...
def random_forest_building():
    ...
    for one_dict in train_list:
        ...
        rfc.fit(one_dict['X'], one_dict['Y'])
        rfc_pred = rfc.predict(test_x)
        one_dict['y_pred'] = rfc_pred
        one_dict['model'] = rfc_pred
        n_errors = (rfc_pred != test_y).sum()
        one_dict['预测错误个数'] = n_errors
        acc = accuracy_score(test_y, rfc_pred)
        prec = precision_score(test_y, rfc_pred)
        rec = recall_score(test_y, rfc_pred)
        f1 = f1_score(test_y, rfc_pred)
        MCC = matthews_corrcoef(test_y, rfc_pred)
        one_final_dict = {
            'name': one_dict['name'],
            '预测错误个数': n_errors,
            '准确率': acc,
